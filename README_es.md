...
- observar regiones continuas de pasto, tierra y bosque; árboles variados que se pueden recolectar; agua ondulante; senderos; reservas comunales; obras en progreso y casas grupales terminadas;
...
- observar debajo del Loop real un flujo Brain v2 simplificado: necesidades, mundo/memoria social, ambos codificadores, fusión y doce Q-values sin la antigua maraña de conexiones;
...
El campeón incluido actualmente es **experimento 041, run 001, seed 42**: alcanzó 5000 ticks con 178 humanos vivos a partir de una población inicial de 20 y 25 animales vivos a partir de 10; la reproducción explica que las poblaciones finales superen las iniciales. El paquete conserva 312 snapshots de brains humanos y 59 animales, ordenados para reutilización. Este campeón histórico fue entrenado con reward v8 y el esquema anterior de 11 acciones e inputs más pequeños. Al iniciar la simulación actual con reward v10, LifeSim amplía los inputs y la salida compatibles de Brain v2 conservando los outputs anteriores, y comienza con Adam, target network, replay personal y Horde replay limpios. Solo un resultado mejor producido con el contrato de aprendizaje actual puede reemplazarlo.
...
checkpoints/best_result_brain/
  registry.json
  champions/experiment_NNN_run_NNN/
    human_*.pt
    animal_*.pt
```
...
El agua se genera en varios clusters grandes y representa fuentes permanentes e intransitables: los agentes beben desde una casilla adyacente y sus rutas rodean el agua. Beber no elimina una casilla. La comida sí se consume y el mundo comienza con una reserva moderada de `food_per_agent` casillas por habitante —una por defecto—. Cada espacio faltante tiene solo un 3% de probabilidad de reaparecer por tick, por lo que una unidad consumida tarda unos 33 ticks en volver en promedio. Tanto las casillas iniciales como los reemplazos se colocan por máxima separación para conservar comida en distintas zonas. Esto hace importante decidir cuándo comer: hacerlo demasiado pronto desperdicia parte del alimento, pero esperar demasiado eleva el riesgo de inanición. El mundo también conserva regiones continuas de pasto, tierra y bosque. Los árboles del bosque son recursos de construcción finitos: recolectar uno lo retira del mapa y añade una unidad de madera cargada.

Cuando no tiene demasiada hambre, un agente puede elegir `GATHER`. La comida cercana continúa viajando a la reserva comunal de su especie; un árbol cercano entrega madera cuando la casa del grupo sigue incompleta. Comida y madera ocupan inventarios separados de una unidad. El agente que carga madera regresa a la obra compartida y elige `BUILD`; ocho aportes completan una casa para toda la especie. Su zona de refugio de cinco casillas no puede ser atacada por depredadores, entrega `+0.01` de energía pasiva por tick, duplica la recuperación al descansar y otorga `+0.30` adicional por descansar con cansancio. Así, la construcción tiene utilidad colectiva duradera y no crea un objeto cosmético privado.
...
Reward v10 organiza hambre y sed en tres zonas. Desde 25% comienza la planificación protegida; el objetivo seguro es permanecer en 50% o menos; 70% abre la zona de peligro. Cada tick por encima del 50% tiene un costo cuadrático que llega a `-0.30` al alcanzar 70% y puede crecer hasta `-0.80`. Volver realmente a la zona segura entrega `+0.25`, además del reward proporcional de comer o beber. Recolectar comida o madera, depositar, acercarse a una tarea de construcción, completar la casa, descansar bajo techo, alimentar una cría y aparearse producen componentes explícitos. Una madre observa el hambre máxima de sus bebés: recibe hasta `-1.50` por tick por hambre superior al 50% y `-5.0` adicionales si una cría dependiente muere de inanición. Como alimentar reduce el hambre antes de calcular el castigo, el brain puede aprender a prevenirlo mediante `GATHER`.

Reward v10 conserva el **gobernador de supervivencia** observable y añade recompensas sociales, de construcción y depredación. El brain produce doce Q-values, pero una máscara elimina acciones físicamente imposibles: no puede elegir `DRINK` sin agua, `EAT` sin comida, `GATHER` con hambre o sin un recurso alcanzable, `BUILD` sin madera junto a una casa grupal incompleta, `MATE` sin pareja elegible, `ATTACK` sin un objetivo legal al alcance, atacar a un ocupante protegido por una casa terminada ni atravesar obstáculos. Entre 25% y 70% el brain decide cuándo consumir; al entrar en peligro, el gobernador fuerza el consumo o una ruta segura hacia el recurso recordado. Durante exploración, epsilon también muestrea solamente acciones permitidas. La experiencia guarda la máscara del siguiente estado y el target DQN excluye los Q-values imposibles antes de calcular su máximo. La web muestra cuándo el gobernador cambió la preferencia original del brain.
...
La suite actual contiene 83 tests y cubre creación del mundo y agentes, biomas visuales agrupados y bosques, comida proporcional distribuida, reposición gradual, recolección de comida y madera, construcción de casas comunales, refugio contra depredadores y descanso grupal reforzado, progreso hacia reservas y depósitos, decisión del momento de comer, sexo F/M, apareamiento, embarazo, camadas, seguimiento, alimentación y castigos maternales, poblaciones ampliadas entre ciclos, ataques de depredadores y defensa humana selectiva, cancelación web, ramas y activaciones de Brain v2, perfiles epsilon individuales, Horde replay por especie, prioridad de necesidades, memoria espacial/social, movimiento y rutas que evitan el agua, comida, bebida, backpropagation con cambio de pesos, checkpoint y reproducción exacta de outputs, selección BRB, rechazo claro de Brain v1 y sanity checks de shapes y límites.
